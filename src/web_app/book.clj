(ns web-app.book
  ...
  (:use [hiccup.form :only [form-to label text-area submit-button]]
        ...
        [web-app.recommendations :only
         [get-similar-books-pearson get-similar-books-euclidean]]))

(defn round-static-rating
  "Prepare static rating stars for dislaying 1/2 and 1 star ratings
  from all users."
  [book]
...
(defn- show-dynamic-rating-stars
  ...
  [:ul.star-rating
   (let [new-rating (session/get :rating)]
     [:li.current-rating
      {:style (str "width:" (calculate-dynamic-star-percentage new-rating)
                   "%;")}
      (str "Currently " new-rating "/5 Stars.")])
   [:li [:a.one-star {:href (str "/rate/" id "&1")
                      :title "1 star out of 5"} "1"]]
   [:li [:a.two-stars {:href (str "/rate/" id "&2")
                       :title "2 stars out of 5"} "2"]]
   [:li [:a.three-stars {:href (str "/rate/" id "&3")
                         :title "3 stars out of 5"} "3"]]
   [:li [:a.four-stars {:href (str "/rate/" id "&4")
                        :title "4 stars out of 5"} "4"]]
   [:li [:a.five-stars {:href (str "/rate/" id "&5")
                        :title "5 stars out of 5"} "5"]]])
  ...
  (let [book (session/get :book)
        ...
        (merge book
               (when-not (or (nil? new-rating) (zero? new-rating))
                 ...
                 {:ratingValue (double (/ (+ (book :ratingValue)
                                             new-rating) 2))})
               {:reviews (conj (book :reviews)
                               (assoc {}
                                 :author user
                                 :authorId
                                 (str (:_id (get-user-by-username user)))
